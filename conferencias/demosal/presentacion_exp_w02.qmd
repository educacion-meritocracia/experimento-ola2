...
plot_tc <- datos %>% filter(p2_2_o2=="De acuerdo") %>% 
  dplyr::select(exp_t4_d_rec:exp_t4_a_rec) %>%
  plot_stackfrq(show.total=FALSE,
                axis.labels=rev(c("A.", "B.", "C.", "D.")),
    geom.colors = c("#E74C3C", "#2E86C1")) + 
  ...
  labs(
    fill = "Cantidad décimas",
    title = "Percepción talento en la escuela"
  )

plot_ts <- datos %>% filter(p1_2_o2=="De acuerdo") %>% 
  dplyr::select(exp_t4_d_rec:exp_t4_a_rec) %>%
  plot_stackfrq(show.total=FALSE,
                axis.labels=rev(c("A.", "B.", "C.", "D.")),
    geom.colors = c("#E74C3C", "#2E86C1")) + 
  ...
  labs(
    fill = "Cantidad décimas",
    title = "Percepción talento en la sociedad"
  )
```
...
ggarrange(plot_tc, plot_ts, common.legend = TRUE, nrow=2, legend = c("bottom", "right"))
...
plot_mc <- datos %>% filter(p2_3_o2=="De acuerdo") %>% 
  dplyr::select(exp_t4_d_rec:exp_t4_a_rec) %>%
  plot_stackfrq(show.total=FALSE,
                axis.labels=rev(c("A.", "B.", "C.", "D.")),
    geom.colors = c("#E74C3C", "#2E86C1")) + 
  ...
  labs(
    fill = "Cantidad décimas",
    title = "Percepción merecimiento en la escuela"
  )

plot_ms <- datos %>% filter(p1_1_o2=="De acuerdo") %>% 
  dplyr::select(exp_t4_d_rec:exp_t4_a_rec) %>%
  plot_stackfrq(show.total=FALSE,
                axis.labels=rev(c("A.", "B.", "C.", "D.")),
    geom.colors = c("#E74C3C", "#2E86C1")) + 
  ...
  labs(
    fill = "Cantidad décimas",
    title = "Percepción merecimiento en la sociedad"
  )
```
...
ggarrange(plot_mc, plot_ms, common.legend = TRUE, nrow=2, legend = c("bottom", "right"))
...
- **Proximamente**: A la luz de estos resultados, se buscará identificar si las creencias meritocráticas de los/as estudiantes están relacionadas con dicha asignación.
