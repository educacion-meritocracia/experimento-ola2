...
datos %>% select(exp_t1_a, exp_t1_b) %>% 
  plot_stackfrq(vjust=rep(c("top", "bottom"),6),
                ...
                axis.labels = c("A.", "B."),
                show.total=FALSE,
                geom.colors = c("#505050", "#555555", "#404040", "#A0A0A0", "#8C8C8C", "#787878")) +
  theme_bw()+
...
datos %>% select(exp_t4_d:exp_t4_a) %>% 
  plot_stackfrq(vjust=rep(c("top", "bottom"),15),
    ...
    geom.colors = c(
     "#505050", "#555555", "#404040", 
  "#C0C0C0", "#D0D0D0", "#B8B8B8",  
  "#A0A0A0", "#909090", "#808080",  
  "#707070", "#606060"   
    )) +
  theme_bw()+
  theme(legend.position="bottom",
...
```{r}
plot_superiores <- datos %>% filter(educ_max=="Estudios superiores") %>% 
  dplyr::select(exp_t4_d_rec:exp_t4_a_rec) %>%
  plot_stackfrq(show.total=FALSE,
                axis.labels=rev(c("A.", "B.", "C.", "D.")),
    geom.colors = c("#5c5c5c", "#DBDBDB"
...
plot_media_menos <- datos %>% filter(educ_max=="Enseñanza media o menos") %>% 
  dplyr::select(exp_t4_d_rec:exp_t4_a_rec) %>%
  plot_stackfrq(show.total=FALSE,
                axis.labels=rev(c("A.", "B.", "C.", "D.")),
    geom.colors = c("#5c5c5c", "#DBDBDB"
...
ggarrange(plot_superiores, plot_media_menos, common.legend = TRUE, nrow=2, legend = c("bottom", "right"))
...
```{r}
plot_mas_lib <- datos %>% filter(libros_hogar=="Más de 25 libros") %>% 
  dplyr::select(exp_t4_d_rec:exp_t4_a_rec) %>%
  plot_stackfrq(show.total=FALSE,
                axis.labels=rev(c("A.", "B.", "C.", "D.")),
    geom.colors = c("#5c5c5c", "#DBDBDB"
  ...
  labs(
    fill = "Cantidad décimas",
    title = "Más de 25 libros"
  )

plot_menos_lib <- datos %>% filter(libros_hogar=="Menos de 25 libros") %>% 
  dplyr::select(exp_t4_d_rec:exp_t4_a_rec) %>%
  plot_stackfrq(show.total=FALSE,
                axis.labels=rev(c("A.", "B.", "C.", "D.")),
    geom.colors = c("#5c5c5c", "#DBDBDB"
...
ggarrange(plot_mas_lib, plot_menos_lib, common.legend = TRUE, nrow=2, legend = c("bottom", "right"))
...
datos <- datos %>% mutate(nse = case_when(
  dependencia == "Colegio Municipal" ~ 1,
  libros_hogar == "Menos de 25 libros"  ~ 1,
  educ_max == "Enseñanza media o menos"  ~ 1,
   dependencia == "Colegio Particular Subvencionado" ~ 2,
